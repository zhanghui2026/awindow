...
test('pairs two browser tabs and opens the transfer workspace', async ({ browser }) => {
  ...
  await expect(creator.locator('.verification-panel')).toContainText('端到端加密已建立')
  await expect(joiner.locator('.verification-panel')).toContainText('端到端加密已建立')
  await expect(creator.locator('.verification-panel')).toContainText(/密钥指纹 [0-9a-f]{4}( [0-9a-f]{4}){3}/u)
  const creatorFingerprint = await creator.locator('.verification-panel').innerText()
  const joinerFingerprint = await joiner.locator('.verification-panel').innerText()
  expect(creatorFingerprint.match(/[0-9a-f]{4}( [0-9a-f]{4}){3}/u)?.[0])
    .toBe(joinerFingerprint.match(/[0-9a-f]{4}( [0-9a-f]{4}){3}/u)?.[0])
