...
- Playwright 覆盖桌面与移动首页、创建房间、密钥交换后的自动验证、二维码公钥认证、Fragment 清理、真实 DataChannel 直连、加密文字直传、加密图片直传与解码、WebSocket 密文回退、刷新恢复，以及同一会话内先后传输文字和图片的综合流程。HTTP 图片回退由客户端协议、HTTP 路由和 WebSocket 通知集成测试覆盖，18 个桌面与移动 E2E 用例全部通过。
